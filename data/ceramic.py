...
workshopdistance_file='data/riverDistances.csv'
...
def getrealdist():
    print("load workshop distance using the file "+workshopdistance_file)
    realdist={}
    with open(workshopdistance_file,'r') as distfile:
...
#compute  means  for all metric
def getmetricsmeans():
    print("compute means for all metrics in file 'data/drespaper.csv'")
    df=pd.read_csv("data/drespaper.csv")
    return(df[df.keys()[4:12]].mean())

#compute  the covariance matrix
def getcovmat():
    print("compute covariance metrics in file 'data/drespaper.csv'")
    df=pd.read_csv("data/drespaper.csv")
    return(df[df.keys()[4:12]].cov())

def getmax():
    print("compute covariance metrics in file 'data/drespaper.csv'")
    df=pd.read_csv("data/drespaper.csv")
    return(df[df.keys()[4:12]].max()*1.5)

def getmin():
    print("compute covariance metrics in file 'data/drespaper.csv'")
    df=pd.read_csv("data/drespaper.csv")
    return(df[df.keys()[4:12]].min()*.5)
def getsds():
    print("compute covariance metrics in file 'data/drespaper.csv'")
    df=pd.read_csv("data/drespaper.csv")
    return(df[df.keys()[4:12]].std()*.1)
