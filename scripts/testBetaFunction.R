...
alphas=10^((seq(-1,1,length.out=100)))
...
plot(1,1,xlim=c(0,100),ylim=c(0,1),type="n",xlab="d",ylab=expression(B(d,alpha)))
sapply(alphas,function(a)lines(d,prs(d,a),lwd=2,col=clrs[as.character(a)]))
for(i in 1:length(alphas)){
    a=alphas[i]
    thisbeta=prs(seq(0,10,length.out=100),a)
